...
* Вам нужно иметь статический (или публичный) IP-адрес и открытый порт.
...
* Если вы не указали http пароль, то возможен доступ только с локального адреса: 127.0.0.1
...
Примерно через 8 секунд транзакция создания поместится в блокчейн и у вас будет открыт счет, на который вы можете майнить монеты 
